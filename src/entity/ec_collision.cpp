...
namespace entity {
    void ECCollision::init(const std::string &filename) {
        ...
        if (mp_mesh->collisions_data().empty()) {
            this->m_collider = std::make_shared<physics::MeshCollider>(physics::ColliderType::Mesh, this->mp_mesh->mesh_data());
        } else {
            auto collision_data = mp_mesh->collisions_data()[0];
            this->m_collider = std::make_shared<physics::MeshCollider>(physics::ColliderType::Mesh, collision_data);
        }
    }
}
